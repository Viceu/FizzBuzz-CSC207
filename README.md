...
strike you as either similar to or different from Python.
  - `and` in python is `&&` in Java
  - there's a `public static void main()` loop in the class outside the for loop
  - java requires {} to mark loops
  - for loop conditions separated by ;
  - `print` in java is `System.out.printIn()`
  - loop names same (for, if, else if, else)
  - calculation syntax the same
...
the file. Then click the commit button to commit the change to the repository. "This is the change"
